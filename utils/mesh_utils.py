...
class GaussianExtractor(object):
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            save_img_u8(self.normals[idx].permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5,
                        os.path.join(vis_path, 'normal_{0:05d}'.format(idx) + ".png"))
            save_img_u8(self.depth_normals[idx].permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5,
                        os.path.join(vis_path, 'depth_normal_{0:05d}'.format(idx) + ".png"))

            # save_img_u8(self.depth_normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'alpha_{0:05d}'.format(idx) + ".png"))

            depth_np = self.depthmaps[idx][0].cpu().numpy()
            depth_vis = vis_depth(depth_np)
            cv2.imwrite(
                os.path.join(vis_path, f"{idx:05d}_depth.png"),
                depth_vis
            )
