...
class GaussianModel:
    ...
    def densification_postfix(self, new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation):
        d = {"xyz": new_xyz,
             "knn_f": new_knn_f,
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacity, new_scaling,
                                   new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        if selected_pts_mask.sum() > 0:
            ...
            stds2 = self.get_scaling[selected_pts_mask]                  # [K,2]
            stds3 = torch.cat([stds2, torch.zeros_like(stds2[:, :1])], 1) # [K,3] 第三维=0，只在平面内抖动
            means = torch.zeros_like(stds3)                               # [K,3]
            samples = torch.normal(mean=means, std=stds3)                 # [K,3]
            ...
            self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities,
    ...
    def add_densification_stats(self, viewspace_point_tensor, viewspace_point_tensor_abs, update_filter):
        self.xyz_gradient_accum[update_filter] = self.xyz_gradient_accum[update_filter] + torch.norm(
            viewspace_point_tensor.grad[update_filter, :2], dim=-1, keepdim=True)
        self.xyz_gradient_accum_abs[update_filter] = self.xyz_gradient_accum_abs[update_filter] + torch.norm(
